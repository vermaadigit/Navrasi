...
const ProductDetail = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await addToCartAPI(product.id, quantity, selectedSize, selectedColor);
        ...
        >
          <ol className="flex items-center gap-2 text-sm flex-wrap">
            <li>
              <Link
                to="/"
                className="text-stone-500 hover:text-stone-900 transition-colors font-light"
              >
                Home
              </Link>
            </li>
            ...
            <li className="text-stone-300">/</li>
            <li className="text-stone-900 font-light truncate max-w-xs">
              {product.title}
            </li>
          </ol>
        </motion.nav>
                ...
                {product.feature && product.feature.length > 0 && (
                  <div className="absolute top-4 left-4 flex flex-col gap-2">
                    {product.feature.map((feat) => (
                      <span
                        key={feat}
                        className={`px-3 py-1 ${getFeatureBadgeColor(
                          feat
                        )} text-xs tracking-wide font-light rounded-sm`}
                      >
                        {feat.toUpperCase()}
                      </span>
                    ))}
